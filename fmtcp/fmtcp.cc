  ...
  if(first_id != subflow_id) {
	  ...
	  while (cur_fmtcp_seqno <= last_ack_ + (int)(wnd_/block_packets) && cur_fmtcp_seqno <= curseq_) {
		  if(!flag && (symbols_actualsent_[offset] < block_packets + append_packets_)){
			  flag = true;
			  appending_block_ = cur_fmtcp_seqno;
			  //char tmp[100]; 
			  //memset(tmp,0,sizeof(tmp));
			  //sprintf(tmp, "appending block %d", appending_block_);
			  //totrace(tmp);
		  }
		  if (symbols_sent_[offset] < block_packets + append_packets_) {
			  ...
			  if(cur_fmtcp_seqno >= appending_block_ && (cur_fmtcp_seqno - appending_block_)<send_buffer_block){
				  scheduled_[cur_fmtcp_seqno - appending_block_]++;  // bound error!!!!
			  } else {
